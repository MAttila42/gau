...
`gau` allows linking accounts even if they have different primary email addresses. When an account is unlinked, `gau` will currently reset the user's `email` to `null` and `emailVerified` to `false`.
