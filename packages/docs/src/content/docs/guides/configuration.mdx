...
> **Type**: `{ defaultRole?: string, resolveOnCreate?: (ctx) => string | undefined, adminRoles?: string[], adminUserIds?: string[] }`,
> **Default**: `{ defaultRole: 'user', adminRoles: ['admin'], adminUserIds: [] }`

Optional role-based configuration. When set, new users will be assigned a role on creation.
Use `resolveOnCreate` to dynamically derive roles (e.g. first-user-is-admin, domain-based),
otherwise `defaultRole` is used. `adminRoles` and `adminUserIds` are used by future helper predicates and are not enforced by core.
