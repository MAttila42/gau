...
1. **Install Tauri Dependencies**
    ...
    <Tabs syncKey="pkg">
      <TabItem label="npm">
        ```sh
        npm i @tauri-apps/api @tauri-apps/plugin-deep-link @tauri-apps/plugin-os @tauri-apps/plugin-shell
        npm i -D @tauri-apps/cli
        ```
      </TabItem>
      <TabItem label="pnpm">
        ```sh
        pnpm add @tauri-apps/api @tauri-apps/plugin-deep-link @tauri-apps/plugin-os @tauri-apps/plugin-shell
        pnpm add -D @tauri-apps/cli
        ```
      </TabItem>
      <TabItem label="yarn">
        ```sh
        yarn add @tauri-apps/api @tauri-apps/plugin-deep-link @tauri-apps/plugin-os @tauri-apps/plugin-shell
        yarn add -D @tauri-apps/cli
        ```
      </TabItem>
      <TabItem label="bun">
        ```sh
        bun add @tauri-apps/api @tauri-apps/plugin-deep-link @tauri-apps/plugin-os @tauri-apps/plugin-shell
        bun add -D @tauri-apps/cli
        ```
      </TabItem>
      <TabItem label="ni">
        ```sh
        ni @tauri-apps/api @tauri-apps/plugin-deep-link @tauri-apps/plugin-os @tauri-apps/plugin-shell
        ni -D @tauri-apps/cli
        ```
      </TabItem>
    </Tabs>
...
2. **Initialize Tauri**
   ...
   If you haven't already, initialize Tauri in your project. This will create a `src-tauri` directory.
...
3. **Configure Deep Linking**
   ...
   In `src-tauri/tauri.conf.json`, add the `deep-link` plugin and define a scheme. The client helpers default to `gau`, but you can configure it. **Set this to something unique to your app.**
...
4. **Set up Rust Backend**
    ...
    Add the Tauri plugins to your `src-tauri/Cargo.toml`.
    ...
    Then, modify `src-tauri/src/lib.rs` (or `main.rs`) to initialize the deep link plugin and listen for incoming URLs. The listener forwards the URL to the frontend via an event.
    ...
    <Tabs syncKey="framework">
        <TabItem label="SvelteKit">
            ...
            export const auth = createSvelteAuth({
              baseUrl: clientEnv.PUBLIC_API_URL, // for dev, use http://localhost:5173/api/auth
            })
            ```
...
6. **Configure the Backend**
   ...
   export const auth = createAuth({
     // ... other options
     trustHosts: 'all', // or ['tauri.localhost']
   })
   ```
