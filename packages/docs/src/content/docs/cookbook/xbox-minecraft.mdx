...
<Aside type="tip" icon="i-ph:lightbulb-duotone">Mark the `xbox` profile as `linkOnly: true` to ensure users can only link it to an existing account and not sign in directly with Microsoft using that profile.</Aside>
