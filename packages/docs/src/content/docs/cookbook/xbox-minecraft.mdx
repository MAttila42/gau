...
- Add the `XboxLive.signin` scope (and `offline_access` if you want refresh tokens)
...
export const auth = createAuth({
  providers: [
    Microsoft({
      ...
      scope: ['openid','profile','email','User.Read','XboxLive.signin','offline_access'],
    })
  ],
...
export const auth = createAuth({
  ...
  onBeforeLinkAccount: async ({ tokens }) => {
    ...
    if (!owns)
      return { allow: false, response: new Response('No Minecraft entitlement', { status: 403 }) }
...
<Aside type="tip" icon="i-ph:lightbulb-duotone">You can also set `linkOnlyProviders: ['microsoft']` to ensure users can only link after sign-in and not sign-in directly with Microsoft, which is typical for Minecraft-style flows.</Aside>
